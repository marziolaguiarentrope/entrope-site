...
import { api, PendingEmail, PendingEmailDetail as PendingEmailDetailData, CommunicationView } from '@/lib/api';
...
interface PendingEmailDetailProps {
  detail: PendingEmailDetailData;
  onClose: () => void;
  onMessageUpdate: (message: PendingEmail) => void;
  renderInline?: boolean;
}

export function PendingEmailDetail({
  detail,
  onClose,
  onMessageUpdate,
  renderInline,
}: PendingEmailDetailProps) {
  ...
  async function handleApprove() {
    ...
    setError(null);
    try {
      const updated = await api.approvePendingEmail(message.id);
  ...
  async function handleReject() {
    ...
    setError(null);
    try {
      const updated = await api.rejectPendingEmail(message.id, reason);
      onMessageUpdate({ ...message, ...updated });
      setShowReject(false);
      setRejectReason('');
    } catch (err) {
      ...
      <div className="p-4 space-y-4">
        <section className="bg-accent/50 rounded-lg p-3 space-y-3">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs font-medium text-muted-foreground">Draft Email</p>
          ...
          <div className="bg-background rounded-lg border border-border p-3">
            <div
              className="text-sm leading-relaxed break-words [&_p]:my-2 [&_a]:text-primary [&_a]:underline"
              dangerouslySetInnerHTML={{ __html: message.body || '<p>—</p>' }}
            />
          </div>
        </section>
        ...
        {!canAct && (
          <div className="bg-accent/30 rounded-lg p-3 text-sm">
            <p className="font-medium">
              {message.approval_status === 'APPROVED' ? 'Email approved' : 'Email rejected'}
            </p>
